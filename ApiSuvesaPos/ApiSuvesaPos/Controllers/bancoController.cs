...
namespace ApiSuvesaPos.Controllers
{
    ...
    public class bancoController : Controller
    {
        ...
        private Negocio.Logica.Bancos db = new Negocio.Logica.Bancos();
        ...
        [HttpPost]
        public IActionResult Registrar(Datos.Models.Banco banco)
        {
            try
            {
                string resp = db.Crear(banco);
        ...
        [HttpPut]
        public IActionResult Actualizar(int id, Datos.Models.Banco banco)
        {
            try
            {

                string resp = db.Editar(id, banco);
                double test;
                if (double.TryParse(resp, out test))// Si el resultado es numerico
                {
                    if (test > 0)//Si el resultado es mayor que cero
